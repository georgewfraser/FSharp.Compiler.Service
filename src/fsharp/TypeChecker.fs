...
and TcModuleOrNamespaceElementsNonMutRec cenv parent typeNames endm (exprsSoFar, attrsSoFar, env: TcEnv, envAtEnd: TcEnv) (moreDefs: SynModuleDecl list) =
  eventually {
    match moreDefs with 
    | (firstDef :: otherDefs) ->
...
/// Recover by searching for the smallest possible expression that includes focus
let rec RecoverModuleOrNamespaceElementNonMutRec (cenv:cenv) parent typeNames scopem synDecl (focus: range) =
      match ElimModuleDoBinding synDecl with
      | SynModuleDecl.NestedModule(compInfo, isRec, mdefs, _, m) ->

          // Treat 'module rec M = ...' as a single mutually recursive definition group 'module M = ...'
          if isRec then 
              let recover env = TcModuleOrNamespaceElementNonMutRec cenv parent typeNames scopem env synDecl
              synDecl.Range.Start, recover
          else
              let (ComponentInfo(_, _, _, _, xml, _, _, _)) = compInfo

              let endm = m.EndRange

              // This is wrong, but I think having a correct ParentRef is not necessary for local typechecking
              let parent = ParentNone

              // Now typecheck. 
              RecoverModuleOrNamespaceElements cenv parent endm xml None mdefs focus

      | SynModuleDecl.NamespaceFragment(SynModuleOrNamespace(longId, isRec, isModule, defs, xml, attribs, vis, m)) when not isRec ->
          ...
          let defs = 
              if isModule then 
                  let modName = List.last longId
                  [SynModuleDecl.NestedModule(ComponentInfo(attribs, [], [], [modName], xml, false, vis, m), false, defs, true, m)] 
              else 
                  defs

          RecoverModuleOrNamespaceElements cenv parent endm xml None defs focus
        // Give up and recompile the entire expression
        | _ -> 
            let recover env = TcModuleOrNamespaceElementNonMutRec cenv parent typeNames scopem env synDecl 
            synDecl.Range.Start, recover
 
/// The non-mutually recursive case for a sequence of declarations
and RecoverModuleOrNamespaceElementsNonMutRec cenv parent typeNames endm (moreDefs: SynModuleDecl list) (focus: range) =
    match moreDefs with 
    // If the focus is within firstDef, recover by re-checking firstDef
    | (firstDef :: _) when rangeContainsRange firstDef.Range focus ->
        // Lookahead one to find out the scope of the next declaration.
        let scopem = unionRanges firstDef.Range endm
        RecoverModuleOrNamespaceElementNonMutRec cenv parent typeNames scopem firstDef focus
    // If the focus is in the later defs, keep looking
    | (firstDef :: otherDefs) when posLt firstDef.Range.End focus.Start ->
        RecoverModuleOrNamespaceElementsNonMutRec cenv parent typeNames endm otherDefs focus
    // Otherwise, give up and re-check what we've got
    | _ -> 
        let start = 
            match moreDefs with 
            | firstDef :: _ -> firstDef.Range.Start 
            | _ -> focus.Start
        let recover env = 
            eventually {
                let! exprs, attrs, env = TcModuleOrNamespaceElementsNonMutRec cenv parent typeNames endm ([], [], env, env) moreDefs 
                return exprs, attrs, env, env
            }
        start, recover

and RecoverModuleOrNamespaceElements cenv parent endm xml mutRecNSInfo defs (focus: range) =
    // Collect the type names so we can implicitly add the compilation suffix to module names
    let typeNames = EstablishTypeDefinitionCores.TypeNamesInNonMutRecDecls defs
    ...
    | Some _ -> 
        ...
        let recover env = 
            eventually {
                let! expr, attrs, env = TcModuleOrNamespaceElements cenv parent endm env xml mutRecNSInfo defs
                return [expr], attrs, env, env
            }
        start, recover
    | None -> 
        RecoverModuleOrNamespaceElementsNonMutRec cenv parent typeNames endm defs focus
...
let TypeCheckOneImplFile 
 ...
 eventually {
    ...
    let! mexpr, topAttrs, envAtEnd = 
        match focus with 
        | None -> TcModuleOrNamespaceElements cenv ParentNone qualNameOfFile.Range envinner PreXmlDocEmpty None defs
        | Some(focus) -> 
            eventually {
                let _, recover = RecoverModuleOrNamespaceElements cenv ParentNone qualNameOfFile.Range PreXmlDocEmpty None defs focus 
                // TODO actually recover TcEnv instead of using envinner
                let! exprs, topAttrs, _, envAtEnd = recover envinner
                return TMDefs(exprs), topAttrs, envAtEnd
            }
